...
export interface CreateActivityViewProps {
...
export default function CreateActivityView({
	...
	const initialSubject = searchParams.get("materia") ?? "";
	...
	/* Seed initial subject from URL on mount */
	useEffect(() => {
		if (initialSubject) {
	...
	async function handleSubmit() {
		...
		try {
			...
			console.error("Create failed in view:", err);
		} finally {
			setSubmitting(false);
		}
	}
		...
		<div className="cav-container fade-in" data-testid="create-activity-view" style={{ animationDelay: "0.2s" }}>
			<div className="cav-card">
				...
				<div className="cav-header">
					<div className="cav-header-left">
						<div className="cav-header-icon">
							<ClipboardList size={20} />
						</div>
						<div className="cav-header-text">
							<h3>Nueva actividad</h3>
							<p className="cav-header-subtitle">Rellena los datos para planificar tu día</p>
						</div>
					</div>
					<button
						className="cav-back-btn"
						onClick={handleGoBackNavigation}
						data-testid="create-activity-back-nav-btn"
					>
						<ArrowLeft size={15} />
						Volver
					</button>
